...
# This is the labsmaterial code.. just changed something to make it clearer.. and better :)
# plus i wrote some stuff for myself of the future
...
class Status(object):
    ...
    def GET(self, *path, **query ): # 127.0.0.1:8080/online/?query=status or 127.0.0.1:8080/online/
        response_dict = {
            'status': 'online',
            'Useful_key': 'Hacked :)'
        }
        # necessary in case of wrong get queries. Try with 127.0.0.1:8080/online/?idontknowwhatimdoing=status
        try:
            response_tmp = json.dumps(response_dict[query])
            response = response_tmp
        except:
            response = json.dumps(response_dict['status'])

        return  response

# endpoint /todos
class TodoList(object):  # http://127.0.0.1:8080/todos/?query=status
    exposed = True
